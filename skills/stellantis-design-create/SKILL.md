...
- The user explicitly asks for a design doc, technical design, architecture, ADRs, UX mockups/wireframes/user flows, a boundary contract (API/UI/CLI/library), a DB schema, architecture diagrams, component-level tests, or a traceability matrix.
- The user wants key technical decisions captured durably (as ADRs) rather than left as chat history.

If an Acceptance Tests document exists from `stellantis-srs-create`, read it before drafting — component-level tests (Phase 11) and traceability (Phase 12) both depend on it.
...
- The SDD's Tech Stack section (Phase 13) summarizes the chosen stack in one table (Concern / Choice / ADR) and links to each ADR for the reasoning; it does not restate the ADR's Alternatives/Consequences inline.
...
- Keep mockups traceable: note which use case(s)/acceptance test(s) each flow and screen mockup corresponds to, feeding directly into Phase 12's traceability matrix.
...
Every SDD must include at least these three diagram types, authored as Mermaid so they render live in the HTML output (Phase 13) instead of being static images that drift out of sync:

- **Component diagram**: the major components/modules/services and the dependencies between them (`graph` or `flowchart`). Direction of arrows should match the dependency direction discussed in Phase 9 (Loose Coupling) — a component pointing at another it doesn't actually depend on is a bug in the diagram, not a stylistic choice.
...
## Phase 10: Functional Core, Imperative Shell
...
## Phase 11: Component-Level Tests (from ATs + Design)

Derive a component-level test plan by combining the Acceptance Tests (Given/When/Then, from `stellantis-srs-create`) with the internal design (Phase 9 components, Phase 10 core/shell split):
...
- Cover every branch/edge case named in the functional core (Phase 10) even if no single AT calls it out explicitly — flag any such gap-filling test as design-driven rather than AT-driven so the distinction is visible.
- Skip generating component tests for trivial passthrough components with no branching logic (consistent with Phase 10's guidance not to force a core split where there's nothing to isolate).
...
- Sections, in this order:
  ...
  8. **Internal Design** — component/module breakdown, each with its single responsibility, dependencies, and coupling notes (Phase 9); the functional-core/imperative-shell split for components with real logic (Phase 10).
  9. **Cross-Cutting Concerns** — auth, error handling/observability, and any other concern that spans components, only if relevant to the requirements in scope.
  10. **Decisions** — a table of ADRs (number, title, status) linking into the ADR directory; don't restate their content.
  11. **Component-Level Tests** — a summary/link to the component test doc (Phase 11); embed the table directly if it's small, otherwise link out.
  12. **Traceability** — the matrix from Phase 12, embedded as an HTML table so it's viewable without opening another file.
  13. **Open Questions / Future Considerations** — anything explicitly deferred, so it's clear it was considered and intentionally excluded from this design pass.
...
- Default file locations, unless the user specifies otherwise:
  ...
  - `docs/design/Component-Tests.md` for the component-level test plan (Phase 11)
...
- After creating the documents, briefly summarize what was produced (ADR count/titles including the tech stack decisions, UX mockups produced if applicable, boundary contract(s) produced and their size, schema table count if applicable, diagram count, component-test count, and any traceability gaps found) rather than repeating full content back in chat.
