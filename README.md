...
| [stellantis-design-create](skills/stellantis-design-create/SKILL.md) | Turns requirements into a lean, HTML Software Design Document (SDD) with rendered component/sequence/deployment diagrams: key decisions (including the tech stack) captured as ADRs, the simplest design that solves the problem, an OpenAPI spec for the API boundary, a database schema for the data boundary, a functional-core/imperative-shell architecture honoring SRP and loose coupling, component-level tests derived from ATs, and a design-to-requirements traceability matrix. |
